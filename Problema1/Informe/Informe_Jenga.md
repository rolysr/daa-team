...
- Si $m > c + e$, entonces solo tiene sentido hacer acciones de quitar y colocar palitos, por lo tanto si $c=e$, el paso de $f(h'-1)$ a $f(h')$ es equivalente ya que para las operaciones necesarias da igual las cantidades que habrían que subir o bajar. Por otro lado, si $c < e$ entonces tiene sentido que al aumentar la cantidad de palitos para poner con respecto a los que hay que quitar y disminuyamos la cantidad de palitos a quitar, tengamos menos costo general de las acciones que en la altura anterior, entonces $f(h'-1) \geq f(h')$. Finalmente, en el caso en que $c > e$, entonces la cantidad de palitos para poner aumenta en el paso de $h'-1$ a $h'$ y disminuye la cantidad de los que hay que quitar, pero en dicho paso todavía no se alcanzan las distribuciones para la cantidad de palitos para subir y bajar que corresponden a la solución óptima. Para este último caso, podemos pensar en que las cantidades de palitos a subir y bajar óptimas son $count_{ups_{op}}$ y $count_{downs_{op}}$ y que lo que se logra con el paso de $h'-1$ a $h'$ sería $count_{ups}'$ y $count_{downs}'$. Luego vemos que al aumentar la altura, $count_{ups}'$ aumenta y se acerca más a $count_{ups_{op}}$, y $count_{downs}'$ disminuye y se acerca más a $count_{downs_{op}}$, por lo tanto con el paso dado, se acerca más el costo $c*count_{ups}'+ count_{downs}'*e$ a $c*count_{ups_{op}} + count_{downs_{op}}*e$, luego podemos afirmar que necesariamente $f(h'-1) \geq f(h')$, demostrando la monotonía decreciente de los valores de evaluación de $f$ para alturas menores que $h$ óptima.

- Si $m \leq c + e$ nótese que podemos reducirlo al caso anterior realizando mientras sea posible las acciones de movimiento, y en este caso la expresión de paso de $h'-1$ a $h'$ podría verse como la cantidad de cada una de las acciones posibles de cada tipo para llegar a la cantidad de acciones posibles para la altura óptima que dan la solución ideal. El razonamiento para $c \leq e$ es idéntico al caso anterior. Sin embargo, para el caso en que $c > e$, nótese que la expresión para la altura óptima $h$ sería $m*moves_{op} + c*adds_{op} + e*dels_{op}$ donde $moves_{op}, adds_{op}$ y $dels_{op}$ son la cantidad de operaciones de movimiento, colocar y eliminar óptimos respectivamente y que de manera análoga al caso anterior, si analizamos la expresión $m*moves' + c*adds' + e*dels'$ correspondiente al paso de $h'-1$ a $h'$, dichos valores se acercan directamente al valor óptimo de costo, ya que disminuye el error de precisión asociado a la cantidad de acciones actuales de cada tipo con respecto a la ideal a medida que aumenta la altura en el paso de $h'-1$ a $h'$, por lo tanto se cumple que $f(h'-1) \geq f(h')$. De esta manera de demuestra la monotonía decreciente de $f$ a la izquierda de $h$ óptima.
...
óptima y su costo mínimo correspondiente se basa en una **Búsqueda Ternaria** con la modificación de que el factor no recursivo de su función $T(m, n)$ de complejidad temporal ahora es lineal y no constante como en su versión clásica, ya que dicho factor es el producto de ejecutar el método $solve(...)$ en $\Omicron(n)$. Finalmente, se tiene que $T(m, n) = T(\frac{2m}{3}) + n$ ya que en cada instancia no base de la *Búsqueda Ternaria*, se realiza un llamado recursivo sobre una sección con tamaño igual a $\frac{2}{3}$ del tamaño del problema en el llamado actual y en el caso no recursivo de realiza una ejecución en $\Omicron{(n)}$. Por lo tanto, si aplicamos el **Teorema Maestro** vemos que $T(m, n) \in \Omicron(n\log{m})$, por lo que esta misma sería la complejidad total del algoritmo.  
